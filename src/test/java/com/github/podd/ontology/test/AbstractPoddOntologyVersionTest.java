...
public abstract class AbstractPoddOntologyVersionTest {
	...
	/**
	 * NOTE: The static OWLOntologyManager instance is reused through all the
	 * tests so that loaded ontologies are kept in memory and able to satisfy
	 * import requirements in later ontologies.
	 * 
	 */
	...
	public static void beforeClass() throws Exception {
		AbstractPoddOntologyVersionTest.owlOntologyManager = OWLOntologyManagerFactoryRegistry
	...
	public AbstractPoddOntologyVersionTest(final String filename,
			final String mimeType, final int statementCount) {
	...
	public void testForValidOWLDLOntology() throws Exception {
		...
		final RDFFormat format = Rio.getParserFormatForMIMEType(this.mimeType);
		...
		Assert.assertEquals("Incorrect number of statements for "
				+ this.filename, this.statementCount, collector.getStatements()
				.size());

		// - proceed to OWL ontology validation

		final RioRDFDocumentFormatFactory ontologyFormatFactory = (RioRDFOntologyFormatFactory) OWLOntologyFormatFactoryRegistry
				.getInstance().getByMIMEType(this.mimeType);
		final RioParserImpl owlParser = new RioParserImpl(ontologyFormatFactory);

		final OWLOntology nextOntology = AbstractPoddOntologyVersionTest.owlOntologyManager
				.createOntology();
		final RioMemoryTripleSource owlSource = new RioMemoryTripleSource(
		...
		final OWLProfile nextProfile = Profiles.OWL2_DL;

		// verify: check ontology in profile
		final OWLProfileReport profileReport = nextProfile
				.checkOntology(nextOntology);
		...
		final OWLReasonerFactory reasonerFactory = Profiles.instantiateFactory(Profiles.Pellet);
		final OWLReasoner reasoner = reasonerFactory
				.createReasoner(nextOntology);

		// Check for any inconsistent classes and if so render them out as
		// explanations
		final Node<OWLClass> unsatisfiableClasses = reasoner
				.getUnsatisfiableClasses();
		final Set<OWLClass> entitiesMinusBottom = unsatisfiableClasses
		...
		if (!entitiesMinusBottom.isEmpty()) {
			...
			for (final OWLClass nextUnsatisfiableClass : entitiesMinusBottom) {
				final ExplanationGenerator explanator = new DefaultExplanationGenerator(
						AbstractPoddOntologyVersionTest.owlOntologyManager,
						reasonerFactory, nextOntology,
						(ExplanationProgressMonitor) null);
				final Set<Set<OWLAxiom>> explanations = explanator
						.getExplanations(nextUnsatisfiableClass);
				out.println("next unsatisfiable class: "
						+ nextUnsatisfiableClass.getIRI());
	...
	/**
	 * Test the ontology can be parsed into RDF and contains the expected number
	 * of statements.
	 */
	@Test
	public void testForValidRDF() throws Exception {
		...
		/*
		 * Assert.assertEquals("Incorrect number of statements for " +
		 * this.filename, this.statementCount, collector
		 * .getStatements().size());
		 */}
